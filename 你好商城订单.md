...
CREATE TABLE `order_physical_goods` (
	...
	`order_status` varchar(1) NOT NULL DEFAULT '0' COMMENT '订单状态（0=未支付；1=待发货；2=待收货；3=交易成功；4=已取消）',
	`pay_status` varchar(1) NOT NULL DEFAULT '0' COMMENT '支付状态（0=未支付；1=已支付；2=已退款）',
	`order_price` float(15,6) NOT NULL COMMENT '订单金额',
	`discount_price` float(15,6) NOT NULL DEFAULT 0 COMMENT '优惠金额',
	`coupon_price` float(15,6) NOT NULL DEFAULT 0 COMMENT '优惠券抵扣金额',
	`actual_price` float(15,6) NOT NULL DEFAULT 0 COMMENT '实付金额',
...
CREATE TABLE `order_physical_goods_detail` (
	...
	`unit_price` float(15,6) NOT NULL COMMENT '生成订单时的单价,单位是元',
	`quantity` int(10) NOT NULL COMMENT '商品数量',
	`total_price` float(15,6) NOT NULL COMMENT '商品总价,单位是元',
	`discount_price` float(15,6) NOT NULL DEFAULT 0 COMMENT '优惠金额',
	`coupon_price` float(15,6) NOT NULL DEFAULT 0 COMMENT '优惠券抵扣金额',
	`actual_price` float(15,6) NOT NULL DEFAULT 0 COMMENT '实付金额',
	`status` varchar(1) NOT NULL DEFAULT '0' COMMENT '状态（0=正常；1=退款；2=退货）',
	`return_time` timestamp NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '退款或退货时间',
	`del_flag` varchar(1) NOT NULL DEFAULT '1' COMMENT '逻辑删除用（置0）',
	`create_id` varchar(20) NOT NULL COMMENT '创建人ID',
	`create_date` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
	`modify_id` varchar(20) DEFAULT NULL COMMENT '修改人ID',
